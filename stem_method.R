...
stem_funnel <- function(beta_input, se_input, stem_estimates){
  ...
  plot(beta_sorted, se_axis, 
       col = "#2e00fa",                          # solid outline
       bg  = adjustcolor("#2e00fa", alpha.f = 0.1),  # transparent fill
       pch = 21, cex = 0.6,
       bty = "l",
       xlim=c(beta_axis_min, beta_axis_max),
       xlab=substitute(paste("Beta estimates"), list(name=labNames[1])),
       ylab=substitute(paste("Precision (1/SE)"), list(name=labNames[2])))#@@@@@@ modify this line if rescaling with a different function    
  #stem (cumulative estimates)
  lines(cumulative_estimates, se_axis, col=rgb(96, 96, 96, maxColorValue = 255), lwd=lineswidth)
  #augment stem
  points(b_stem, se_axis[n_stem], pch=filled_diamond ,col="darkblue", cex = points_size)
  segments(b_stem, se_axis[1], b_stem, se_axis_min,col="darkblue", lwd=lineswidth)
  #stem-based estimate
  points(b_stem, se_axis[1], pch=filled_diamond ,col="#ca0086", cex = points_size)
  segments(b_stem-t_stat*SE_b_stem, se_axis[1], b_stem+t_stat*SE_b_stem, se_axis[1],col="#ca0086", lwd=lineswidth)
  #zero line
  ...
  legend("topleft", #@@@@@@ modify this line if want to put legend in a different location
         ...
         col = c("#ca0086" , 
                 "#ca0086" ,
                 col=rgb(96, 96, 96, maxColorValue = 255) ,
                 "darkblue" ,  
                 "#2e00fa") , 
         bty = "n", 
